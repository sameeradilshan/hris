...
class UserManagement_model extends CI_model{
	...
	class HrUser_model extends CI_model{	
...
public function getuserProfileData($adminId) {
	...
	$this->db->select('*');
	$this->db->from('hruser d');
	$this->db->where('d.hrUserId',$adminId);
	$query = $this->db->get();
	return $query->result();
}
//---------------------------------------Leave data view----------------------------------------
...
/////////////////////////////////////////////////////////////
			public function getpasswordChange($newPasswd, $wherearray){
				//var_dump($newPasswd);
				$result=$this->db->query('UPDATE hruser SET  hrUserPassword = "'.$newPasswd.'"  WHERE  hrUserId =  "'.$wherearray.'" ;');
				return $result;
				//var_dump($result);
